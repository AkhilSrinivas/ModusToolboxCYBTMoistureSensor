...
/** @file
 *
 * This demo application shows a  implementation of a moisture sensor.
 * The app is based on the snip/mesh/mesh_sensor_server sample which
 * implements BLE Mesh Sensor Server model.
 *
 * Features demonstrated
 *  - moisture measurement using the on board moisture on the EVK
 *
 * To demonstrate the app, walk through the following steps.
 * 1. Build and download the application (to the WICED board)
 * 2. Use Android MeshController and provision the moisture sensor
 ...
 * 4. To change the moisture on the moisture, you can keep your finger on the sensor and see the changes.
 */
...
// The onboard moisture hardware has a positive and negative tolerance of 1%
...
static long int       mesh_sensor_get_moisture_8(void);
...
long int        mesh_sensor_current_moisture = 0;
long int        mesh_sensor_sent_value = 0;           //
uint32_t      mesh_sensor_sent_time;                // time stamp when moisture was published
...
// Optional setting for the moisture sensor, the Total Device Runtime, in Time Hour 24 format
uint8_t mesh_moisture_sensor_setting0_val[] = { 0x01, 0x00, 0x00 };
...
{
    {
        ...
        .val                 = mesh_moisture_sensor_setting0_val
    },
};
...
{
    {
        ...
        .data = (int*)&mesh_sensor_sent_value,
        .cadence =
        {
...
{
    {
        ...
        .default_level = 0,                                              // Default value of the variable controlled on this element (for example power, lightness, moisture, hue...)
        .range_min = 1,                                                  // Minimum value of the variable controlled on this element (for example power, lightness, moisture, hue...)
        .range_max = 0xffff,                                             // Maximum value of the variable controlled on this element (for example power, lightness, moisture, hue...)
...
void mesh_app_init(wiced_bool_t is_provisioned)
{
    ...
    wiced_bt_cfg_settings.device_name = (uint8_t *)"Soil Moisture Sensor";
    ...
    // read the initial moisture
    mesh_sensor_current_moisture = mesh_sensor_get_moisture_8();
    ...
    mesh_sensor_sent_value = mesh_sensor_current_moisture;
...
void mesh_sensor_server_restart_timer(wiced_bt_mesh_core_config_sensor_t *p_sensor)
{
    ...
    {
        // The moisture is not interrupt driven.  If client configured sensor to send notification when
    ...
    {
        // If fast cadence period divisor is set, we need to check moisture more
...
/*
 */
long int mesh_sensor_get_moisture_8(void)
{
    long int temp_celsius_100 = moisture_read();

    return temp_celsius_100;
}
...
void mesh_sensor_server_report_handler(uint16_t event, uint8_t element_idx, void *p_get, void *p_ref_data)
{
    ...
    case WICED_BT_MESH_SENSOR_GET:
        // measure the moisture and update it to mesh_config
        mesh_sensor_sent_value = mesh_sensor_get_moisture_8();
    ...
    else
    {
        ...
        if (!pub_needed && ((p_sensor->cadence.trigger_delta_up != 0) || (p_sensor->cadence.trigger_delta_down != 0)))
        {
            if (!p_sensor->cadence.trigger_type_percentage)
            {
                WICED_BT_TRACE("Native cur value:%d sent:%d delta:%d/%d\n",
                        mesh_sensor_current_moisture, mesh_sensor_sent_value, p_sensor->cadence.trigger_delta_up, p_sensor->cadence.trigger_delta_down);

                if (((p_sensor->cadence.trigger_delta_up != 0)   && (mesh_sensor_current_moisture >= (mesh_sensor_sent_value + p_sensor->cadence.trigger_delta_up))) ||
                    ((p_sensor->cadence.trigger_delta_down != 0) && (mesh_sensor_current_moisture <= (mesh_sensor_sent_value - p_sensor->cadence.trigger_delta_down))))
                {
            ...
            else
            {
                ...
                if ((p_sensor->cadence.trigger_delta_up != 0) && (mesh_sensor_current_moisture > mesh_sensor_sent_value))
                {
                    WICED_BT_TRACE("Delta up:%d\n", ((uint32_t)(mesh_sensor_current_moisture - mesh_sensor_sent_value) * 10000 / mesh_sensor_current_moisture));
                    if (((uint32_t)(mesh_sensor_current_moisture - mesh_sensor_sent_value) * 10000 / mesh_sensor_current_moisture) > p_sensor->cadence.trigger_delta_up)
                    {
                        WICED_BT_TRACE("Pub needed percent delta up:%d\n", ((mesh_sensor_current_moisture - mesh_sensor_sent_value) * 10000 / mesh_sensor_current_moisture));
                        pub_needed = WICED_TRUE;
                    }
                }
                else if ((p_sensor->cadence.trigger_delta_down != 0) && (mesh_sensor_current_moisture < mesh_sensor_sent_value))
                {
                    WICED_BT_TRACE("Delta down:%d\n", ((uint32_t)(mesh_sensor_sent_value - mesh_sensor_current_moisture) * 10000 / mesh_sensor_current_moisture));
                    if (((uint32_t)(mesh_sensor_sent_value - mesh_sensor_current_moisture) * 10000 / mesh_sensor_current_moisture) > p_sensor->cadence.trigger_delta_down)
                    {
                        WICED_BT_TRACE("Pub needed percent delta down:%d\n", ((mesh_sensor_sent_value - mesh_sensor_current_moisture) * 10000 / mesh_sensor_current_moisture));
                        pub_needed = WICED_TRUE;
                    }
                }
            }
        ...
        if (!pub_needed && (mesh_sensor_fast_publish_period != 0))
        {
            ...
            if (cur_time - mesh_sensor_sent_time >= mesh_sensor_fast_publish_period)
            {
                ...
                {
                    if ((mesh_sensor_current_moisture >= p_sensor->cadence.fast_cadence_low) &&
                        (mesh_sensor_current_moisture <= p_sensor->cadence.fast_cadence_high))
                    {
        ...
        {
           if (((p_sensor->cadence.trigger_delta_up == 0) && (mesh_sensor_current_moisture > mesh_sensor_sent_value)) ||
               ((p_sensor->cadence.trigger_delta_down == 0) && (mesh_sensor_current_moisture < mesh_sensor_sent_value)))
            {
